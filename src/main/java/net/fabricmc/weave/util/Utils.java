...
public final class Utils {
    ...
    private static void addAllPotentialAncestors(JarIndex jarIndex, Set<ClassEntry> classEntries, ClassEntry classObfEntry) {
        Iterator superClassEntry = jarIndex.getTranslationIndex().getInterfaces(classObfEntry).iterator();

        while(superClassEntry.hasNext()) {
            ClassEntry interfaceEntry = (ClassEntry)superClassEntry.next();
    ...
    public static boolean isBehaviorProvider(JarIndex jarIndex, ClassEntry classObfEntry, BehaviorEntry behaviorEntry) {
        if(behaviorEntry instanceof MethodEntry) {
            MethodEntry methodEntry = (MethodEntry)behaviorEntry;
            HashSet classEntries = new HashSet();
            addAllPotentialAncestors(jarIndex, classEntries, classObfEntry);
            Iterator var5 = classEntries.iterator();

            while(var5.hasNext()) {
                ClassEntry parentEntry = (ClassEntry)var5.next();
    ...
    public static String[] serializeEntry(Entry entry, boolean removeNone, String... extraFields) {
        ...
        if (entry instanceof FieldEntry) {
            ...
            data[2] = ((FieldEntry) entry).getType().toString();
            ...
            data[0] = "METHOD";
            data[1] = entry.getClassName();
            data[2] = ((MethodEntry) entry).getSignature().toString();
            data[3] = entry.getName();
    ...
    public static Entry deserializeEntry(String[] data) {
        if (data.length > 0) {
            if (data[0].equals("FIELD") && data.length >= 4) {
                return new FieldEntry(new ClassEntry(data[1]), data[3], new Type(data[2]));
            } else if (data[0].equals("METHOD") && data.length >= 4) {
                return new MethodEntry(new ClassEntry(data[1]), data[3], new Signature(data[2]));
